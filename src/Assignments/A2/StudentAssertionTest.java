    ...
    public static void testFitnessCouncil_Negative() {
       ...
       //stress test your codes.
        ...
        FitnessCentre duplicate =
                new FitnessCentre("ironworks", "FC-002");

        assert council.registerCentre(c1)
                : "First centre should register successfully";

        // Duplicate name, ignoring capitalisation
        assert !council.registerCentre(duplicate)
                : "Duplicate centre name should be rejected";

        // Centre does not exist
        assert council.getClassesByCentre("Unknown Centre").isEmpty()
                : "Unknown centre should return an empty list";

        assert council.getActivitiesByCentre("Unknown Centre").isEmpty()
                : "Unknown centre should return an empty map";

        // Activity does not exist
        assert council.getClassesByActivity("Boxing").isEmpty()
